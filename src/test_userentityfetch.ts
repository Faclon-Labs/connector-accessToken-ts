import { UserAccess, UserDetailsHeaders, UserEntityFetchResponse } from './connectors/UserAccess.js';

async function testUserEntityFetch() {
  ...
  const entityFetchResponse: UserEntityFetchResponse | null = await userAccess.getUserEntityFetch(extraHeaders);
  console.log('User entity fetch response:', entityFetchResponse);
}
