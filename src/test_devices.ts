import { DeviceAccess, DeviceHeaders, Device } from './connectors/DeviceAccess.js';
...
async function testGetAllDevices() {
  ...
  try {
    const devices: Device[] | null = await deviceAccess.getAllDevices(extraHeaders);
    if (devices) {
      console.log('Raw API Response:');
      console.log(JSON.stringify(devices, null, 2)); // Pretty print with 2 space indentation
    } else {
      console.log('Failed to retrieve devices');
    }
  } catch (error) {
    console.error('Error testing getAllDevices:', error);
  }
}
