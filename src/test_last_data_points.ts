import { DeviceAccess, DeviceHeaders, LastDataPointsResponse } from './connectors/DeviceAccess.js';

async function testGetLastDataPoints() {
  ...
  try {
    const lastDataPoints: LastDataPointsResponse | null = await deviceAccess.getLastDataPoints(extraHeaders);
    if (lastDataPoints) {
      console.log('Raw Last Data Points Response:');
      console.log(JSON.stringify(lastDataPoints, null, 2)); // Pretty print with 2 space indentation
    } else {
      console.log('Failed to retrieve last data points');
    }
  } catch (error) {
    console.error('Error testing getLastDataPoints:', error);
  }
}

testGetLastDataPoints(); 