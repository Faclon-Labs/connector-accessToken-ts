import { DeviceAccess, DeviceHeaders, DeviceByDevIDResponse } from './connectors/DeviceAccess.js';

async function testGetDeviceByDevID() {
  ...
  const devID = 'F2401_A12'; // Replace with an actual device devID
  ...
  try {
    const device: DeviceByDevIDResponse | null = await deviceAccess.getDeviceByDevID(devID, extraHeaders);
    if (device) {
      console.log('Raw Device by DevID Response:');
      console.log(JSON.stringify(device, null, 2)); // Pretty print with 2 space indentation
    } else {
      console.log('Failed to retrieve device by devID');
    }
  } catch (error) {
    console.error('Error testing getDeviceByDevID:', error);
  }
}

testGetDeviceByDevID(); 